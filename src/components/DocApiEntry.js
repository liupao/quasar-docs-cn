...
function getPropDetails (openState, masterKey, prop, level) {
  ...
  if (prop.returns !== void 0 && prop.returns !== null) {
    details.push(
      getDiv(
        12,
        `返回值类型：${getStringType(prop.returns.type)}`,
        void 0,
        h(
          'div',
...
function getProp (openState, masterKey, prop, propName, level, onlyChildren) {
  ...
  child.push(
    ...getExpandable(
      openState,
      language.value === 'en' ? prop.desc : (prop.desc_cn || prop.desc + '（该 api 还未翻译）'),
      isExpandable,
      childKey,
  ...
  for (const eventName in events) {
    ...
    child.push(
      h('div', { class: 'doc-api-entry row' }, [
        ...
        ...getExpandable(
          openState,
          language.value === 'en' ? event.desc : (event.desc_cn || event.desc + '（该 api 还未翻译）'),
          event.params !== void 0,
          masterKey,
          () => {
            const params = []
...
describe.methods = (openState, methods) => {
  ...
  for (const methodName in methods) {
    ...
    const methodNode = h('div', { class: 'doc-api-entry row' }, [
      ...
      ...getExpandable(
        openState,
        language.value === 'en' ? method.desc : (method.desc_cn || method.desc + '（该 api 还未翻译）'),
        ...
        () => {
          ...
          if (method.returns !== void 0) {
            nodes.push(
              getDiv(
                12,
                `返回值类型：${getStringType(method.returns.type)}`,
                void 0,
                h(
                  'div',
                  { class: 'doc-api-entry__subitem' },
